...
my-application$ sam local invoke getStockMaterialsFunction --event events/event-get-all-items.json
```
...
Resources:
  ...
  getAllItemsFunction:
    ...
    Properties:
      Handler: src/handlers/get-all-items.getStockMaterialsHandler
      Runtime: nodejs10.x
      DeadLetterQueue:
        Type: SQS 
...
```bash
aws cloudformation delete-stack --stack-name masisa-materials-stock
```
